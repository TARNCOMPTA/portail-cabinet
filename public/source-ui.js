...
function initSourceUI({ prefix: P, source, label, profession = false, tousDocuments = false }) {
  ...
  async function charger() {
    try { clients = await api(ep('/clients')); } catch { return; }
    const n = clients.length;
    if (el('compte')) el('compte').textContent = n ? `${n}` : '';
    const navc = document.getElementById(`nav-${source}-count`); if (navc) navc.textContent = n || '';
    rendre(); chargerRuns();
  }
  function rendre() {
    const q = filtre.toLowerCase();
    const liste = q ? clients.filter((c) => `${c.nom} ${c.login} ${profession ? (PRO[c.profession] || '') : ''} ${c.notes || ''}`.toLowerCase().includes(q)) : clients;
  ...
  async function chargerRuns() {
    try {
      ...
      if (tb) tb.innerHTML = runs.slice(0, 100).map((r) => `
  ...
  $(`#table-${source}-clients`).addEventListener('click', async (e) => {
    ...
    if (a) { e.preventDefault(); return voirDocs(Number(a.dataset[`${P}Docs`])); }
  ...
  async function supprimer(id) {
    ...
    try { await api(ep(`/clients/${id}`), { method: 'DELETE' }); toast('Client supprimé.', 'ok'); charger(); }
    catch (err) { toast(err.message, 'err'); }
  }
    ...
    try {
      ...
      $('#docs-liste').innerHTML = docs.length
        ? docs.map((d) => `<li><span class="lib">${esc(d.libelle || d.fichier.split(/[\\/]/).pop())}</span>
            <a class="btn small" href="${ep(`/documents/${d.id}/file`)}" target="_blank" rel="noopener">Ouvrir</a></li>`).join('')
  ...
  el('import-btn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      el('import-texte').value = ''; charger();
    } catch (err) { toast(err.message, 'err'); }
  });

  el('recherche')?.addEventListener('input', (e) => { filtre = e.target.value.trim(); page = 1; rendre(); });
