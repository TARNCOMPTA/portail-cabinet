// Logique de la section CARMF. Charge APRES app.js : reutilise $, api, toast, esc.
(() => {
  let clients = [];
  let filtre = '';
  let page = 1;
  ...
  async function charger() {
    try { clients = await api('/api/carmf/clients'); } catch { return; }
    const n = clients.length;
    $('#cm-compte').textContent = n ? `${n}` : '';
    const navc = document.getElementById('nav-carmf-count'); if (navc) navc.textContent = n || '';
    rendre();
    chargerRuns();
  }
  function rendre() {
    ...
    if (page < 1) page = 1;
    const tb = $('#table-carmf-clients tbody');
    tb.innerHTML = liste.slice((page - 1) * PAR_PAGE, page * PAR_PAGE).map((c) => `
      <tr${c.verrouille ? ' style="background:var(--err-bg);"' : ''}>
        ...
        <td>${esc(c.notes || '')}</td>
        <td>${c.nb_docs ? `<a href="#" data-cm-docs="${c.id}" style="color:var(--accent);font-weight:600;text-decoration:none;">${c.nb_docs}</a>` : '0'}</td>
        <td>${statutBadge(c.dernier_statut)} <span class="aide" style="margin:0;">${fmtDate(c.dernier_run)}</span></td>
        <td><span class="row-actions">
          <button class="btn small primary" data-cm-scrape="${c.id}">Récupérer</button>
          <button class="btn small" data-cm-edit="${c.id}">Modifier</button>
          <button class="btn small danger" data-cm-del="${c.id}">✕</button>
        </span></td>
      </tr>`).join('');
    $('#cm-vide').hidden = liste.length !== 0;
  ...
  async function chargerRuns() {
    try {
      const runs = await api('/api/carmf/runs');
      $('#table-carmf-runs tbody').innerHTML = runs.slice(0, 100).map((r) => `
  ...
  const form = $('#form-carmf');
  function resetForm() {
    form.reset(); form.id.value = '';
    $('#cm-form-titre').textContent = 'Ajouter un client CARMF';
    $('#cm-submit').textContent = 'Enregistrer';
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      if (id) {
        ...
        await api(`/api/carmf/clients/${id}`, { method: 'PUT', body: JSON.stringify(payload) });
        toast('Client mis à jour.', 'ok');
      } else {
        await api('/api/carmf/clients', { method: 'POST', body: JSON.stringify(payload) });
  ...
  $('#table-carmf-clients').addEventListener('click', async (e) => {
    const a = e.target.closest('[data-cm-docs]');
    if (a) { e.preventDefault(); return voirDocs(Number(a.dataset.cmDocs)); }
    const btn = e.target.closest('button[data-cm-scrape], button[data-cm-edit], button[data-cm-del]');
    if (!btn) return;
    if (btn.dataset.cmScrape) return recuperer(Number(btn.dataset.cmScrape));
    if (btn.dataset.cmEdit) return editer(Number(btn.dataset.cmEdit));
    if (btn.dataset.cmDel) return supprimer(Number(btn.dataset.cmDel));
  });
  ...
  function editer(id) {
    ...
    $('#cm-form-titre').textContent = `Modifier — ${c.nom}`;
    $('#cm-submit').textContent = 'Mettre à jour';
    $('#cm-cancel').hidden = false;
    form.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }
  async function supprimer(id) {
    const c = clients.find((x) => x.id === id);
    if (!confirm(`Supprimer le client CARMF « ${c?.nom || id} » ?\n(Les PDF déjà téléchargés ne sont pas supprimés.)`)) return;
    try { await api(`/api/carmf/clients/${id}`, { method: 'DELETE' }); toast('Client supprimé.', 'ok'); charger(); }
    catch (err) { toast(err.message, 'err'); }
  }
  async function recuperer(id, force = false) {
    try {
      await api(`/api/carmf/clients/${id}/scrape`, { method: 'POST', body: JSON.stringify({ force }) });
  ...
  $('#cm-scrape-all').addEventListener('click', async () => {
    if (!clients.length) return toast('Aucun client CARMF.', 'err');
    if (!confirm('Lancer la récupération CARMF pour TOUS les clients (en série) ?\nLes comptes verrouillés sont ignorés.')) return;
    try {
      const r = await api('/api/carmf/scrape-all', { method: 'POST', body: JSON.stringify({}) });
      let msg = `Récupération lancée : ${r.total} client(s).`;
      if (r.ignores?.length) msg += ` ${r.ignores.length} verrouillé(s) ignoré(s).`;
      toast(msg, 'ok');
      $('#cm-stop').hidden = false; $('#cm-scrape-all').disabled = true;
    } catch (err) { toast(err.message, 'err'); }
  });
  $('#cm-stop').addEventListener('click', async () => {
  ...
  async function voirDocs(id) {
    const c = clients.find((x) => x.id === id);
    try {
      const docs = await api(`/api/carmf/clients/${id}/documents`);
      $('#docs-titre').textContent = `Documents CARMF — ${c?.nom || ''}`;
      $('#docs-liste').innerHTML = docs.length
        ? docs.map((d) => `<li><span class="lib">${esc(d.libelle || d.fichier.split(/[\\/]/).pop())}</span>
            <a class="btn small" href="/api/carmf/documents/${d.id}/file" target="_blank" rel="noopener">Ouvrir</a></li>`).join('')
  ...
  $('#cm-import-btn').addEventListener('click', async () => {
    const rows = parseImport($('#cm-import-texte').value);
    if (!rows.length) return toast('Rien à importer.', 'err');
    try {
      const b = await api('/api/carmf/clients/import', { method: 'POST', body: JSON.stringify({ clients: rows }) });
      let msg = `${b.crees} créé(s), ${b.maj} mis à jour`;
      if (b.erreurs?.length) msg += `, ${b.erreurs.length} erreur(s)`;
      $('#cm-import-bilan').textContent = msg;
      toast(msg, b.erreurs?.length ? 'err' : 'ok');
      $('#cm-import-texte').value = ''; charger();
    } catch (err) { toast(err.message, 'err'); }
  });

  $('#cm-recherche').addEventListener('input', (e) => { filtre = e.target.value.trim(); page = 1; rendre(); });

  charger();
  setInterval(() => {
    api('/api/status').then((s) => {
      const actif = Array.isArray(s.enCours) && s.enCours.some((k) => String(k).startsWith('carmf'));
      $('#cm-scrape-all').disabled = actif;
      $('#cm-stop').hidden = !actif;
    }).catch(() => {});
    charger();
  }, 6000);
})();
