...
(() => {
  ...
  async function chargerCabinets() {
    try { cabinets = await api('/api/urssaf/cabinets'); } catch { return; }
    $('#table-urssaf-cabinets').hidden = cabinets.length === 0;
    $('#u-cab-vide').hidden = cabinets.length !== 0;
    $('#table-urssaf-cabinets tbody').innerHTML = cabinets.map((c) => `
    ...
    $('#u-filtre-cabinet').innerHTML = '<option value="">Tous les comptes</option>' + cabinets.map((c) => `<option value="${c.id}">${esc(c.libelle || c.login)}</option>`).join('');
  }

  const formCab = $('#form-urssaf-cabinet');
  function resetCab() { formCab.reset(); formCab.id.value = ''; $('#ucab-submit').textContent = 'Ajouter le compte'; $('#ucab-cancel').hidden = true; }
  ...
  formCab.addEventListener('submit', async (e) => {
    ...
    btn.disabled = true;
    try {
      if (id) { await api(`/api/urssaf/cabinets/${id}`, { method: 'PUT', body: JSON.stringify(payload) }); toast('Compte mis à jour.', 'ok'); }
      else { await api('/api/urssaf/cabinets', { method: 'POST', body: JSON.stringify(payload) }); toast('Compte ajouté.', 'ok'); }
      resetCab(); chargerCabinets();
  ...
  $('#table-urssaf-cabinets').addEventListener('click', async (e) => {
    ...
    if (btn.dataset.ucab === 'sync') {
      btn.disabled = true; const t = btn.textContent; btn.textContent = '↻ Sync…';
      try {
        ...
        toast(msg, 'ok');
        chargerCabinets(); chargerClients();
      } catch (err) { toast(err.message, 'err'); }
      ...
      $('#ucab-submit').textContent = 'Mettre à jour'; $('#ucab-cancel').hidden = false;
      ...
      try { await api(`/api/urssaf/cabinets/${id}`, { method: 'DELETE' }); toast('Compte supprimé.', 'ok'); chargerCabinets(); chargerClients(); }
      catch (err) { toast(err.message, 'err'); }
    }
  });
  ...
  async function chargerClients() {
    try { clients = await api('/api/urssaf/clients'); } catch { return; }
    const n = clients.length;
    $('#u-compte').textContent = n ? `${n}` : '';
    const navc = document.getElementById('nav-urssaf-count'); if (navc) navc.textContent = n || '';
    rendre();
    chargerRuns();
  }
  function rendre() {
  ...
  async function chargerRuns() {
    try {
      ...
      $('#table-urssaf-runs tbody').innerHTML = runs.slice(0, 100).map((r) => `
        <tr><td>${fmtDate(r.lance_le)}</td><td>${esc(r.client_nom || '—')}</td><td>${statutBadge(r.statut)}</td><td>${r.nb_docs ?? 0}</td><td class="aide" style="margin:0;">${esc(r.message || '')}</td></tr>`).join('');
    } catch { /* ignore */ }
  }

  const formCli = $('#form-urssaf-client');
  function resetCli() { formCli.reset(); formCli.id.value = ''; $('#u-submit').textContent = 'Enregistrer le client'; $('#u-cancel').hidden = true; }
  $('#u-cancel').addEventListener('click', resetCli);
  formCli.addEventListener('submit', async (e) => {
    e.preventDefault();
    const id = formCli.id.value;
    const payload = { nom: formCli.nom.value.trim(), siret: formCli.siret.value.trim(), cabinet_id: formCli.cabinet_id.value ? Number(formCli.cabinet_id.value) : null };
    if (!payload.nom || !payload.siret) return toast('Nom et SIRET requis.', 'err');
    try {
      if (id) { await api(`/api/urssaf/clients/${id}`, { method: 'PUT', body: JSON.stringify(payload) }); toast('Client mis à jour.', 'ok'); }
      else { await api('/api/urssaf/clients', { method: 'POST', body: JSON.stringify(payload) }); toast('Client ajouté.', 'ok'); }
      resetCli(); chargerClients();
    } catch (err) { toast(err.message, 'err'); }
  });

  $('#table-urssaf-clients').addEventListener('click', async (e) => {
    const a = e.target.closest('[data-u-docs]');
    if (a) { e.preventDefault(); return voirDocs(Number(a.dataset.uDocs)); }
    ...
    if (btn.dataset.uEdit) {
      ...
      if (!c) return;
      formCli.id.value = c.id; formCli.nom.value = c.nom; formCli.siret.value = c.siret; formCli.cabinet_id.value = c.cabinet_id || '';
      $('#u-submit').textContent = 'Mettre à jour'; $('#u-cancel').hidden = false;
    ...
    if (btn.dataset.uDel) {
      ...
      try { await api(`/api/urssaf/clients/${btn.dataset.uDel}`, { method: 'DELETE' }); toast('Client supprimé.', 'ok'); chargerClients(); }
      catch (err) { toast(err.message, 'err'); }
    }
  });

  async function recuperer(id) {
    try { await api(`/api/urssaf/clients/${id}/scrape`, { method: 'POST', body: JSON.stringify({}) }); toast('Récupération lancée — suis la progression en haut.', 'ok'); }
    catch (err) { toast(err.message, 'err'); }
  }
  $('#u-scrape-all').addEventListener('click', async () => {
    ...
    try {
      ...
      $('#u-stop').hidden = false; $('#u-scrape-all').disabled = true;
    } catch (err) { toast(err.message, 'err'); }
  });
    ...
    try {
      ...
      $('#docs-liste').innerHTML = docs.length
        ? docs.map((d) => `<li><span class="lib">${esc(d.libelle || d.fichier.split(/[\\/]/).pop())}</span>
            <a class="btn small" href="/api/urssaf/documents/${d.id}/file" target="_blank" rel="noopener">Ouvrir</a></li>`).join('')
  ...
  function parseImport(texte) {
    ...
    if (head.some((h) => /nom|siret|compte|identifiant/.test(h))) {
      cols = { nom: -1, siret: -1 };
      head.forEach((h, i) => { if (/nom/.test(h)) cols.nom = i; else if (/siret|compte|identifiant/.test(h)) cols.siret = i; });
      lignes.shift();
    }
    return lignes.map((l) => { const v = cut(l); return cols ? { nom: v[cols.nom] || '', siret: v[cols.siret] || '' } : { nom: v[0] || '', siret: v[1] || '' }; });
  }
  $('#u-import-btn').addEventListener('click', async () => {
    ...
    try {
      ...
      $('#u-import-texte').value = ''; chargerClients();
    } catch (err) { toast(err.message, 'err'); }
  });
