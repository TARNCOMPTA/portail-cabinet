  ...
  try {
    ...
    for (const c of cibles) {
      try {
        ...
        if (!tokResp.ok()) {
          log(`(${c.libelle} : indisponible — HTTP ${tokResp.status()})`);
          continue;
        }
        const token = (await tokResp.text()).trim().replace(/^"|"$/g, '');
        if (!token || /[<{>]/.test(token)) {
          log(`(${c.libelle} : jeton PDF invalide)`);
          continue;
        }
        ...
        if (!pdfResp.ok()) {
          log(`(${c.libelle} : téléchargement HTTP ${pdfResp.status()})`);
          continue;
        }
        const buf = await pdfResp.body();
        if (buf.length < 100 || buf.subarray(0, 4).toString() !== '%PDF') {
          log(`(${c.libelle} : réponse non-PDF)`);
          continue;
        }
