...
export function listUsers() {
  return db.prepare('SELECT id, email, nom, role, actif, created_at, last_login FROM users ORDER BY email COLLATE NOCASE')
    .all().map((u) => ({ ...u, actif: !!u.actif }));
}
...
export function createUser({ email, nom, password_hash, role }) {
  const info = db.prepare('INSERT INTO users (email, nom, password_hash, role) VALUES (?, ?, ?, ?)')
...
export function setUserActif(id, actif) { db.prepare('UPDATE users SET actif = ? WHERE id = ?').run(actif ? 1 : 0, id); }
...
export function touchUserLogin(id) { db.prepare("UPDATE users SET last_login = datetime('now') WHERE id = ?").run(id); }
...
export function getSessionUser(token) {
  const r = db.prepare(`SELECT u.id, u.email, u.nom, u.role, u.actif, s.expire_le
    FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.token = ?`).get(token);
  if (!r) return null;
  if (Date.parse(r.expire_le) < Date.now()) { deleteSession(token); return null; }
...
export function deleteUserSessions(userId) { db.prepare('DELETE FROM sessions WHERE user_id = ?').run(userId); }
export function purgerSessionsExpirees() { db.prepare("DELETE FROM sessions WHERE expire_le < datetime('now')").run(); }
