// Base SQLite CARPV (Caisse Autonome de Retraite et de Prévoyance des Vétérinaires).
// Un client = un identifiant adherent (identifiant + mot de passe chiffre). Fichier SEPARE
// (carpv.db). Meme modele que CARMF (connexion par client, une seule profession).
...
const db = new DatabaseSync(resolve(DATA_DIR, 'carpv.db'));
...
export function importClients(rows) {
  ...
  rows.forEach((r, i) => {
    ...
    try {
      if (existant) { updateClient(existant.id, { nom, login, password: password || undefined, notes }); bilan.maj++; }
      else {
